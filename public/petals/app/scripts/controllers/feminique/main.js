 ...
 */
angular.module('petalsApp').controller('FeminiqueCtrl', function ($scope, $http, myDateService, $rootScope) {

  $scope.feminique = [];

  $rootScope.url_id = '2';

  $scope.loadFeminque = function (){

    $scope.promise = $http.get('api/get_feminique').then(function(response){
      //callNotification('Your project has been successfully created. Hang on, taking you to your project now','notice');
      //NProgress.done();
      $scope.feminique = response.data.fashions;
  ...
  $scope.deleteFeminique = function (id, $event){
        ...
        buttons: {
              somethingElse: {
                ...
                action: function(){
                  ...
                  $scope.promise = $http.post('api/delete_feminique', {payload: $scope.post_data}).then(function(response){
...
angular.module('petalsApp').controller('EditFeminiqueCtrl', function ($scope, $http, myDateService, $routeParams, FileUploader, $rootScope) {
  ...
  $scope.loadArticle = function (){
    ...
    $scope.promise = $http.post('api/get_single_feminique', {payload: $scope.post_data}).then(function(response){
      //callNotification('Your project has been successfully created. Hang on, taking you to your project now','notice');
      //NProgress.done();
  ...
  $scope.editFeminque = function (){
    ...
    $scope.promise = $http.post('api/edit_feminique', {payload: $scope.post_data}).then(function(response){
      NProgress.done();
  ...
  var uploader = $scope.uploader = new FileUploader({
            url: 'api/edit_fem_picture?id=' + $routeParams.purposeid,
            autoUpload: true
        });
